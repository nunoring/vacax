...
2026-07-02 | 합성용 reference.webp 15종 전부 얼굴 없는 2x3 헤어 시트(정면/측면/후면/상단 마네킹)로 재생성. display 모델컷과 reference 합성용 이미지를 명확히 분리. 새 reference로 앱 3컷 재테스트($0.12)했으나 GPT Image 2는 여전히 1/3번 얼굴 재구성 큼, 2번만 상대적 양호. 다음 후보: 편집 모델/마스크/프롬프트 추가 강화.
2026-07-03 | Claude 진단 반영: GPT Image 2 전체 재생성 한계 확인, Kontext 단일 이미지 편집 실험 준비. 단일 이미지 프롬프트 분기에 style_prompt/negative/modeHint 누락 보정, `kontext-once/three` env 모드 추가. typecheck/build 통과, `eec7467` push.
2026-07-03 | FLUX Kontext Pro 3컷 실제 테스트(소프트 아이비/시스루 댄디/세미리프, 총 $0.12). 생성 후 즉시 mock disarm. 얼굴 보존 기대보다 낮음: 1/3번 얼굴 재구성, 2번은 수염/인상 변화 발생. 모델 스왑 단독 해결 실패, 다음 후보는 마스크/합성백으로 얼굴 영역 원본 픽셀 보존.
2026-07-02 | Claude 얼굴 보존 실패 진단: gpt-image-2 전체 재생성(픽셀 보존 불가)이 근본 원인, 프롬프트로 해결 불가 판정. 1순위 flux-kontext 스왑(.env만, 코드 경로 이미 존재), 2순위 마스크+원본 합성백. 유료 호출 0.
2026-07-03 | Composite-back postprocess tested with prior Kontext outputs, no paid calls. Original face/background pixels were restored outside generated hair mask, then face-restore overlay tried. Result: identity preserved better, but hairline/seam artifacts too severe for product use. App fixture `?hfFixture=app3` updated locally for visual review; keep fixture/run_logs uncommitted.
2026-07-03 | Replicate `flux-kontext-apps/change-haircut` tested as best current generation route. 3-cut test + 3-angle Side-Parted test + app proxy path test completed, total new paid calls $0.44. Result: better than GPT Image 2/Kontext/composite-back for identity/background, but style control is coarse enum-based. Added `hairfit:haircut-once/three` and app proxy mapping in code commit `11adfe3`; server reset to mock/stub.
2026-07-03 | Side-angle photo gate fixed. Face analysis/recommendation is now treated as front-photo only: yaw beyond threshold blocks progression with "정면 사진 필요" copy instead of allowing estimated recommendation/generation. CEO 3 photos verified in browser: front ready, two side-angle photos blocked. Code commit `3b4d7c7` pushed; paid calls 0.
