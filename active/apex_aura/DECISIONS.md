...
## [2026-05-29] 앱 이름 닮은꼴 찾기 → Apex Aura 환원 (2026-05-22 결정 번복)
- CEO 확정 ("의도한결정"). Manifest label + main.dart title + 스토어 등록정보 동시 교체
- 2026-05-22 "Apex Aura → 닮은꼴 찾기" 한글 네이밍 결정을 되돌림
...
## [2026-05-23] Claude 호출 = 2단계 호출 구조 영구 채택
- 1차(메인): first_impression + comparison + consultant_report + action_cards (~20초)
- 2차(styling, 백그라운드): makeup_steps + fashion_looks (~10초)
- 효과: 첫 화면 진입 시간 50% 단축 + 잘림/timeout 위험 분산
- 이유: 1회 호출은 maxTokens 한도 끝까지 차서 잘림 반복

## [2026-05-23] JSON balancer 영구 채택
- max_tokens 잘림 시 마지막 콤마 위치까지 자르고 누락 } ] 보완 → 부분 파싱 성공
- 이유: Claude는 schema 필드 있으면 maxTokens 끝까지 다 씀 → 한도 풀면 또 끝까지 → 본질적 trade-off, balancer로 우회

## [2026-05-23] 컨설팅 도구 박스 + 셀럽 카탈로그 prompt 인라인 박음
- 도구: 삼정비/황금비/광대 위치/턱선 곡률/헤어 5분류(남/여)/컬러 시즌 4타입/K-뷰티 4단계/남성 그루밍 4단계/패션 실루엣 5분류
- 카탈로그: 한국 솔로 배우·가수 30명(남15/여15) + 스타일 키워드
- 효과: Claude가 측정 기준 사용 + references 정확 매칭 (박형식의 2021년 이태원클라쓰 스타일 등 구체 응답)
- 이유: Claude는 도메인 지식 충분히 가지지만 prompt가 활용 유도 안 하면 일반론

## [2026-05-23] 페이지 4 "3박자 솔루션" → "컨설팅 리포트" 재배치
- 이전: 페이지 2가 길고 페이지 4가 빈 화면 (three_factor 제거 영향)
- 변경: 페이지 2 = 점수+강점+셀럽 (간결), 페이지 4 = 단점+솔루션+액션카드+갭 축소 배너 (컨설팅 본론)
- 이유: 사용자 페이지 흐름 평가 — 컨설팅 본론을 별도 페이지로 분리하면 깊이 ↑

## [2026-05-23] 남성도 가벼운 화장 추천
- 이전: "메이크업 대신 스킨케어+눈썹+헤어왁스"
- 변경: BB크림/톤업크림/컨실러 가벼운 화장 OK + 그루밍 4단계 병행
- 이유: 한국 남성 그루밍 시장 — 가벼운 화장이 표준화됨
