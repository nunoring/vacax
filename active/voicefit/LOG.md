...
- 말투 저장/선택 기능 추가: 온보딩/대시보드/생성 화면이 선택 말투를 localStorage+URL로 공유하고, 생성 type 링크 반영까지 검증했다.
- 앱 실험 결과 상품글이 두 말투 모두 72점 고착. 원인을 `CURATION_COPY`/`HOOK_RULES`와 상품글 2-pass 미적용으로 판정하고 D-023으로 기록했다.
- 상품글 생성 구조 수정: 광고 규칙 재주입 제거, product 2-pass+자동 보정 적용, commerce_pack 블로그 섹션 교체 유틸 추가. 앱 재검증 78점/검수 이동/정책 감사 pass.
